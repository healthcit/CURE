...
public class DataExporter {
	...
	public Cure constructModuleXML(long id)
	{
		...
		for(BaseForm form: forms)
		{
			...
			if(!form.isLibraryForm())
			{
				...
				if(parent != null)
				{
					xmlSection.setParentId(parent.getUuid());
					xmlSection.setRelationshipName(((QuestionnaireForm)form).getRelationshipName());
				}
	...
	private Form processForm(BaseForm qForm, Cure rootElement)
	{
		...
		form.setMultipleInstances(((QuestionnaireForm) qForm).isMultipleInstances());
//		if(qForm instanceof QuestionnaireForm && ((QuestionnaireForm) qForm).getParent() != null){			
//			form.setParentId(((QuestionnaireForm) qForm).getParent().getUuid());
//			form.setRelationshipName(((QuestionnaireForm) qForm).getRelationshipName());			
//		}
