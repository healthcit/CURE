...
public class ModuleManager {
	...
	public List<Module> getAllFlatModules(){
		return moduleDao.getFlatModuleList();
	}
...
//						+ " isUserModuleAuthor = " + isUserModuleAuthor
	...
	public Module copyModule(Module moduleToCopy) {
		...
		if(roleCodes.contains(RoleCode.ROLE_ADMIN) || roleCodes.contains(RoleCode.ROLE_LIBRARIAN)) {
			...
			List<BaseForm> orphanForms = formManager.getAllOrphanForms(moduleToCopy.getId());
			List<QuestionnaireForm> formsToCopy = new ArrayList();
			for (BaseForm form: orphanForms)
			{
				formsToCopy.add((QuestionnaireForm)form);
			}
			...
			copyForms(formsToCopy, copiedModule, null, formLibraryModule, oldFormIdsNewFormIdsMap, oldAnswerValueIdsNewAnswerValueIdsMap);
...
//			}
	...
	private void copyForms (List<QuestionnaireForm> formsToCopy, Module copiedModule, QuestionnaireForm parentForm, FormLibraryModule formLibraryModule, HashMap<Long, Long> oldFormIdsNewFormIdsMap, HashMap<String, String> oldAnswerValueIdsNewAnswerValueIdsMap)
	{
		for (QuestionnaireForm formToCopy : formsToCopy) {
			QuestionnaireForm questionnaireFormToCopy = (QuestionnaireForm) formToCopy;
			FormLibraryForm importedFormLibraryForm = questionnaireFormToCopy.getFormLibraryForm(); 
			if(importedFormLibraryForm == null) {
				importedFormLibraryForm = (FormLibraryForm) formManager.importFormToModule(formLibraryModule, formToCopy, true, oldAnswerValueIdsNewAnswerValueIdsMap);
			...
			if(questionnaireFormToCopy.getFormSkipRule() != null) {
				...
				if(parentForm != null)
				{
					copiedForm.setParent(parentForm);
				}
				formManager.updateFormLibraryForm(copiedForm, importedFormLibraryForm);
			}
			copyForms(((QuestionnaireForm)formToCopy).getChildren(), copiedModule, copiedForm, formLibraryModule, oldFormIdsNewFormIdsMap, oldAnswerValueIdsNewAnswerValueIdsMap);
		}
	}
