...
public class FormDao
{
	...
	public List<BaseForm> getModuleForms(Long moduleId) {
		//String jpql = "select frm from QuestionnaireForm frm " +
			//"left join fetch frm.elements where frm.module.id = :moduleId group by frm.parent.id order by frm.ord";
	...
	public List<BaseForm> getAllOrphanForms(long moduleId)
	{
//		TypedQuery<QuestionnaireForm> q = this.em.createQuery("FROM QuestionnaireForm form ORDER BY form.ord ASC", QuestionnaireForm.class);
//		return q.getResultList();
		
		String jpql = "select distinct frm from BaseForm frm " +
				"left join fetch frm.elements where frm.module.id = :moduleId and frm.parent.id is NULL order by frm.ord";
			Query query = em.createQuery(jpql);
			query.setParameter("moduleId", moduleId);
			//query.setParameter("formId", formId);
	...
	public Collection<QuestionnaireForm> getAllChildrenForms(long moduleId, long formId)
	{
//		TypedQuery<QuestionnaireForm> q = this.em.createQuery("FROM QuestionnaireForm form ORDER BY form.ord ASC", QuestionnaireForm.class);
//		return q.getResultList();
		
		String jpql = "select frm from QuestionnaireForm frm " +
				"left join fetch frm.elements where frm.module.id = :moduleId and frm.parent.id  = :formId order by frm.ord";
			Query query = em.createQuery(jpql);
			query.setParameter("moduleId", moduleId);
			query.setParameter("formId", formId);
