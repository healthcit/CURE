...
public class CADSRManager {
	private static Logger log = LoggerFactory.getLogger( CADSRManager.class );
	...
	private static enum CADSRSearchType { SINGLEIDSEARCH, MULTIIDSEARCH, TEXTSEARCH, SINGLEPUBLICIDSEARCH, CDECARTUSERSEARCH };
		...
		try {
			CADSRSearchType caDSRSearchType = 
			 ( questionSearchType == FormElementSearchCriteria.SEARCH_BY_CADSR_TEXT      ? CADSRSearchType.TEXTSEARCH :
			   questionSearchType == FormElementSearchCriteria.SEARCH_BY_CADSR_CART_USER ? CADSRSearchType.CDECARTUSERSEARCH :
			   null );			
