...
public class GenerateSampleDataController {
	...
	private static Logger log = LoggerFactory.getLogger( GenerateSampleDataController.class );
	...
	{
		//TODO: Currently only handles "flat" modules.
		// In the future, modify the Data Generator code to handle hierarchical modules as well.
		return moduleManager.getAllFlatModules();
	}
	...
	public ModelAndView submitForm( @ModelAttribute GeneratedModuleDataDetail formDetail )
	{
		...
		try
		{
			...
			modelAndView.getModelMap().put( "numDocumentsGenerated", formDetail.getActualNumberOfCouchDbForms() );
		}
		catch(Exception ex)
		{
			ex.printStackTrace();
