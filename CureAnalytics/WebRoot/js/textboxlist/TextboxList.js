...
jQuery.TextboxList = function(element, _options){
    ...
    plugins: {},
		...
		encode: function(o){ 
			return jQuery.grep(jQuery.map(o, function(v){		
				v = (chk(v[0]) ? v[0] : v[1]);
	...
	var init = function(){		
		...
		container = jQuery('<div class="'+options.prefix+'" />')
			.insertAfter(element)
			.click(function(e){ 
		...
		list = jQuery('<ul class="'+ options.prefix +'-bits" />').appendTo(container);
		for (var name in options.plugins) enablePlugin(name, options.plugins[name]);		
		afterInit();
	};
	
	var enablePlugin = function(name, options){
		self.plugins[name] = new jQuery.TextboxList[camelCase(capitalize(name))](self, options);
	};
	...
	var afterInit = function(){
		...
		jQuery(document).click(function(e){
			...
			if (e.target.className.indexOf(options.prefix) != -1){				
				if (e.target == jQuery(container).get(0)) return;				
				var parent = jQuery(e.target).parents('div.' + options.prefix);
			...
			var special = !!jQuery.map(['shift', 'alt', 'meta', 'ctrl'], function(e){ return ev[e]; }).length;
	...
	var create = function(klass, value, opt){
		if (klass == 'box'){
			...
			if (options.unique && jQuery.inArray(uniqueValue(value), index) != -1) return false;		
		}		
		return new jQuery.TextboxListBit(klass, value, self, jQuery.extend(true, options.bitsOptions[klass], opt));
	};
	...
	var focusRelative = function(dir, to){
		var el = getBit(to && jQuery(to).length ? to : current).toElement();
		var b = getBit(el[dir]());
		if (b) b.focus();
		return self;
	};
	...
	var getBit = function(obj){				
		return (obj.type && (obj.type == 'editable' || obj.type == 'box')) ? obj : jQuery(obj).data('textboxlist:bit');
	};
	...
	var setValues = function(values){
		...
		jQuery.each(values, function(i, v){
			if (v) add.apply(self, jQuery.isArray(v) ? [v[1], v[0], v[2]] : [v]);
		});		
	};
	...
	var addEvent = function(type, fn){
		...
		var exists = false;
		jQuery.each(events[type], function(f){
			if (f === fn){
				exists = true;
				return;
			};
	...
	var fireEvent = function(type, args, delay){
		...
		jQuery.each(events[type], function(i, fn){		
			(function(){
	...
	var isDuplicate = function(v){
		return jQuery.inArray(uniqueValue(v), index);
	};
	...
	this.setOptions = function(opt){
		options = jQuery.extend(true, options, opt);
	};
...
jQuery.TextboxListBit = function(type, value, textboxlist, _options){
	
	var element, bit, prefix, typeprefix, close, hidden, focused = false, name = capitalize(type); 
	var options = jQuery.extend(true, type == 'box' ? {
		deleteButton: true
  } : {
		tabIndex: null,
		growing: true,
	...
	var init = function(){
		...
		bit = jQuery('<li />').addClass(prefix).addClass(typeprefix)
		...
		if (type == 'box'){
			...
			if (options.deleteButton){
				...
				close = jQuery('<a href="#" class="'+ typeprefix +'-deletebutton" />').click(remove).appendTo(bit);
			}
			bit.children().click(function(e){ e.stopPropagation(); e.preventDefault(); });
		} else {
			element = jQuery('<input type="text" class="'+ typeprefix +'-input" autocomplete="off" />').val(self.value ? self.value[1] : '').appendTo(bit);
			if (chk(options.tabIndex)) element.tabIndex = options.tabIndex;
			if (options.growing) new jQuery.GrowingInput(element, options.growingOptions);		
			...
			if (options.addKeys || options.stopEnter){
				element.keydown(function(ev){
					...
					if (jQuery.inArray(ev.which, splat(options.addKeys)) != -1){
						evStop();
						toBox();
					}
				});
...
var splat = function(a){ return jQuery.isArray(a) ? a : [a]; };
...
jQuery.fn.extend({
	...
	textboxlist: function(options){
		return this.each(function(){
			new jQuery.TextboxList(this, options);
		});
	}
