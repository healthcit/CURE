...
if(C){return C(B);}if(typeof A=="string"){return document.newElement(A,B);}return document.id(A).set(B);},afterImplement:function(A,B){Element.Prototype[A]=B;if(Array[A]){return ;
}Elements.implement(A,function(){var C=[],G=true;for(var E=0,D=this.length;E<D;E++){var F=this[E][A].apply(this[E],arguments);C.push(F);if(G){G=($type(F)=="element");
}}return(G)?new Elements(C):C;});}});Element.Prototype={$family:{name:"element"}};Element.Constructors=new Hash;var IFrame=new Native({name:"IFrame",generics:false,initialize:function(){var E=Array.link(arguments,{properties:Object.type,iframe:$defined});
var C=E.properties||{};var B=document.id(E.iframe)||false;var D=C.onload||$empty;delete C.onload;C.id=C.name=$pick(C.id,C.name,B.id,B.name,"IFrame_"+$time());B=new Element(B||"iframe",C);
...
Native.implement([Element,Document],{getElement:function(A,B){return document.id(this.getElements(A,true)[0]||null,B);},getElements:function(A,D){A=A.split(",");
...
}};if(O){var P=R.getElementsByTagName("*"),Q=this.getElementsByTagName("*");for(var M=P.length;M--;){N(P[M],Q[M]);}}N(R,this);return document.id(R);},destroy:function(){Element.empty(this);
...
},this);return this;},cloneEvents:function(D,A){D=document.id(D);var C=D.retrieve("events");if(!C){return this;}if(!A){for(var B in C){this.cloneEvents(D,B);}}else{if(C[A]){C[A].keys.each(function(E){this.addEvent(A,E);
...
var H={x:L.x-I.x,y:L.y-I.y};var J=(K&&(K=document.id(K)))?K.getPosition():{x:0,y:0};return{x:H.x-J.x,y:H.y-J.y};},getCoordinates:function(J){if(B(this)){return this.getWindow().getCoordinates();
...
(function(){($try(function(){A.doScroll("left");return document.id(A).inject(document.body).set("html","temp").dispose();}))?B():arguments.callee.delay(50);})();
...
},initialize:function(L,M){this.instance="Swiff_"+$time();this.setOptions(M);M=this.options;var B=this.id=M.id||this.instance;var A=document.id(M.container);Swiff.CallBacks[this.instance]={};
...
B.callChain();}.bind(this));return this;}});Fx.Morph=new Class({Extends:Fx.CSS,initialize:function(B,A){this.element=this.subject=document.id(B);this.parent(A);},set:function(A){if(typeof A=="string"){A=this.search(A);
...
I=$extend({data:D.data,url:D.url,method:D.method},I);var E=I.data,B=I.url,A=I.method;switch($type(E)){case"element":E=document.id(E).toQueryString();break;case"object":case"hash":E=Hash.toQueryString(E);
...
}if(C.update){document.id(C.update).empty().set("html",B.html);}if(C.evalScripts){$exec(B.javascript);}this.onSuccess(B.tree,B.elements,B.html,B.javascript);}});
